const bodyweightExercises = {
    "Legs": [
        ...
        { name: "Wal Sit", baseDuration: 30, instruction: "Hold parallel to the floor against a flat surface." }
    ],
    "Push/Chest": [
...
const muscleGroups = ["Legs", "Push/Chest", "Core", "Back"];
...
function generateDailyWorkout() {
    ...
    const level = onboarding.experience_level || 'Novice';
    const timeSlot = onboarding.available_time || 30; // Minutes

    // Determine number of exercises based on rank
    ...
    // Scale intensity/reps based on time and level
    // Time slots: 20 (<30), 45 (30-60), 90 (60+)
    ...
    const daySeed = new Date().getDate() % muscleGroups.length;
    const muscleGroup = muscleGroups[daySeed];
    const pool = bodyweightExercises[muscleGroup];

    const selectedBodyweight = [];
    ...
    for (let i = 0; i < bodyweightCount && poolIndices.length > 0; i++) {
        ...
        selectedBodyweight.push(ex);
    }

    const selectedBoxing = [];
    ...
    for (let i = 0; i < boxingCount && boxingIndices.length > 0; i++) {
        ...
        if (ex.baseDuration) {
            // Boxing durations are usually in rounds (3 mins = 180s)
            // Scale rounds instead of raw seconds for boxing
...
function getDailyWorkout() {
    ...
    let baseWorkout = generateDailyWorkout();

    // 2. Merge Deployed Planner Drills
    ...
    if (plannerDrillsRaw) {
        try {
            ...
            // Prefix names to distinguish them
            const processedPlannerDrills = plannerDrills.map(d => ({
                name: d.title, // Use title as name
                duration: parseInt(d.duration) || 180, // Fallback round
                type: "timer",
                isPlanner: true,
                impact: d.impact,
                instruction: "Precision Protocol from AI Roadmap."
            }));
    ...
    // 3. Check for Tactical Session (Overrides or appends?)
    // User said "includes in daily grind", so I'm appending to daily.
