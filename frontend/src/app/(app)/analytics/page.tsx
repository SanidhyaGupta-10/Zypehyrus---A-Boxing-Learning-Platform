...
export default function AnalyticsPage() {
  ...
  const handleExportHUD = () => {
    ...
    const report = `🥊 ZEPHYR AI COMBAT HUD\n` +
      `----------------------------\n` +
    ...
    <div className="flex flex-col gap-6 anim-fade-in relative pb-16">
      ...
      <div className="flex border border-white/5 bg-white/[0.02] p-1.5 rounded-full select-none">
        <button
          ...
          className={`flex-1 py-3 text-xs font-black uppercase tracking-wider rounded-full transition-all duration-300 ${
            mode === 'daily' 
                      ...
                      {visionHistory.map((item, idx) => {
                        ...
                        return (
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-full border text-[9px] font-bold uppercase tracking-wider flex-shrink-0 transition-all ${
                              selectedVisionIdx === idx
