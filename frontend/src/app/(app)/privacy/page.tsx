      ...
      <div className="flex flex-col gap-4">
        {[
          ...
          },
          {
            ...
            desc: 'When generating training roadmaps, your profile details are sent to the ZEPHYR AI engine via a secure uplink. No biometric photos or video streams are ever uploaded.'
          },
          {
          ...
          <GlassCard 
            key={idx} 
            className={`p-5 flex gap-4 items-start border-white/5 bg-black/40 ${
              point.danger ? 'border-red-500/10 bg-red-500/[0.01]' : ''
            }`}
          >
            <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
              point.danger ? 'bg-red-500/10 text-red-500' : 'bg-primary/10 text-primary'
            }`}>
              {point.icon}
            </div>
            <div>
              <h4 className={`text-xs font-black uppercase tracking-wide leading-none mb-1.5 ${
                point.danger ? 'text-red-500' : 'text-white'
              }`}>
                {point.title}
              </h4>
