...
const Dominance: React.FC = () => {
    ...
    const selectedGoal = (data as any).primary_goal || 'Aerial';
    ...
    return (
        <div className="flex flex-col min-h-[85vh] justify-between py-2">
            ...
            <main className="grid grid-cols-2 gap-4 py-4">
                {goals.map((goal) => (
                    ...
                    >
                        ...
                        <div className={`w-12 h-12 rounded-full border flex items-center justify-center mb-4 transition-all ${selectedGoal === goal.id ? 'border-primary bg-primary/10 shadow-[0_0_20px_rgba(var(--primary-rgb),0.3)] w-14 h-14' : 'border-white/5 bg-white/5'
                            }`}>
                            {goal.icon}
                        </div>
                        <span className={`text-[10px] font-black italic tracking-widest uppercase ${selectedGoal === goal.id ? 'text-primary' : 'text-white'}`}>
                            {goal.label}
                        </span>
                    </button>
                ))}
            ...
            <footer className="mt-8 flex flex-col gap-4">
                <button onClick={nextStep} className="btn-primary w-full h-[75px] text-xl font-black italic uppercase tracking-wider shadow-[0_10px_40px_rgba(var(--primary-rgb),0.3)]">
