...
const Experience: React.FC = () => {
    ...
    const selectedRank = (data as any).experience_level;
        ...
        <div className="flex flex-col min-h-[85vh] justify-between py-2">
            ...
            <main className="flex-1 flex flex-col gap-3 py-4">
                {ranks.map((rank) => (
                    <button
                        key={rank.id}
                        onClick={() => updateData({ experience_level: rank.id } as any)}
                        className={`flex items-center justify-between p-6 rounded-3xl border transition-all duration-300 glass-card ${selectedRank === rank.id
                    ...
                    >
                        ...
                        <div className={`w-10 h-10 rounded-full border flex items-center justify-center font-black text-sm ${selectedRank === rank.id ? 'border-primary bg-primary text-black' : 'border-white/10 text-white/20'
                            }`}>
                            {rank.num}
                        </div>
                    </button>
